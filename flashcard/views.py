...
def deletar_flashcard(request, id):
    ...
    if request.user.id == flashcard.user.id:
        flashcard.delete()
        messages.add_message(request, messages.SUCCESS, 'Flashcard deletado com sucesso!')
        return redirect('/flashcard/novo_flashcard/')
    else:
        messages.add_message(request, messages.ERROR, 'O Flashcard a ser deletado não pertence ao usuário logado!')
        return redirect('/flashcard/novo_flashcard/')
...
def iniciar_desafio(request):
    if request.method == "GET":
        ...
        for categoria in categorias:
            desafio.categoria.add(categoria) #adiciona os id das categorias, não o nome

        #utilizando filtro de lockup. (filtre pelas categorias, onde os ids delas esteja dentro de uma lista de ids de categorias)
        flashcards = (
            Flashcard.objects.filter(user=request.user)
            .filter(dificuldade=dificuldade)
            .filter(categoria_id__in=categorias)
            .order_by('?') #pode ser utilizado com id (ordem crescente), -id (ordem decrescente), ? (ordem aleatória) etc.
        )

        if flashcards.count() < int(qtd_perguntas):
            messages.add_message(request, messages.ERROR, 'A quantidade de questões informada é maior que a quantidade de Flashcards!')
            return redirect('/flashcard/iniciar_desafio/')

        flashcards = flashcards[: int(qtd_perguntas)] #realizando um slice a partir de quantas perguntas o usuário informar.
                                                      #porém para n puxar sempre as mesmas perguntas em ordem, foi utilizado o order_by
                                                      #na criação do flashcard logo acima
...
def relatorio(request, id):
    ...
    return render(request, 'relatorio.html', {'desafio': desafio, 'dados': dados, 'categorias': nome_categoria, 'dados2': dados2})